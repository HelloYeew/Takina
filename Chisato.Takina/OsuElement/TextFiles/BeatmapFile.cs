...
using Chisato.Takina.OsuElement.Components.Events;
using Chisato.Takina.OsuElement.Components.HitObjects;

namespace Chisato.Takina.OsuElement.TextFiles
{
    public partial class BeatmapFile
    {
        ...
        public static BeatmapFile Read(Stream stream)
        {
            ...
            if (!int.TryParse(r.ReadLine()?.Replace("osu file format v", string.Empty), out file.FileFormatVersion))
                throw new Exception("Not a valid beatmap"); // TODO: Very simple check, could be better
